...
function Todo() {
    ...
    useEffect(() => {
        axios.get('http://localhost:5173/getTodoList')
            .then(result => {
    ...
    const addTask = (e) => {
        ...
        axios.post('http:localhost:5173/addTodoList', { task: newTask, status: newStatus })
            .then(res => {
                console.log(res);
    ...
    const deleteTask = (id) => {
        axios.delete('http://localhost:5173/deleteTodoList/' + id)
            .then(result => {
                console.log(result);
                window.location.reload();
            })
